...
import {is_activeService} from 'assets';
...
const SearchCoupon: ForwardRefRenderFunction<refProps, props> = (
  props: props,
    ...
    <Drawer
      ...
      <Form layout="vertical" className="h-full relative" onFinish={onFinish}>
        <Row className="d-block h-full overflow-auto px-4 pb-24">
          ...
          <Form.Item name="code" label={t('code')} className="mb-1/2 label-p-0" initialValue={queryObject?.code}>
            <Input placeholder={t('empty')} className="w-full" />
          </Form.Item>
          <Form.Item
            name="is_active"
            ...
            <SimpleSelect
              ...
              allowClear
              alignDropDownTop
              data={map(is_activeService, (value) => ({...value, name_fa: t(value?.name)}))}
            />
          </Form.Item>
        </Row>
        <Row
...
export default forwardRef(SearchCoupon);
