...
import {convertTimeToLocalMoment, convertTimeToUTC, queryStringToObject} from 'utils';
      ...
      <Form layout="vertical" className="h-full relative" onFinish={onFinish}>
        <Row className="d-block h-full overflow-auto px-4 pb-24">
          ...
          <Form.Item
            ...
            initialValue={queryObject?.question_groups}>
            <MultiSelectPaginate
          ...
          <Form.Item
            ...
            label={t('question')}
            className="mb-1/2 label-p-0"
            initialValue={queryObject?.questions}>
            <MultiSelectPaginate
              mode="multiple"
          ...
          <Form.Item
            ...
            className="mb-1/2 label-p-0"
            initialValue={queryObject?.is_answered}>
            <SimpleSelect keys="id" label="name" placeholder={t('all')} allowClear data={AnsweredType} />
          </Form.Item>
          <Form.Item
            noStyle
            shouldUpdate={(prevValues, nextValues) => prevValues?.is_answered !== nextValues?.is_answered}>
            {() =>
              form.getFieldValue('is_answered') && (
                <Form.Item
                  name="answered_at"
                  label={t('answered_at')}
                  className="mb-1/2 label-p-0"
                  initialValue={queryObject?.answered_at}>
                  <DateTimePicker />
                </Form.Item>
              )
            }
