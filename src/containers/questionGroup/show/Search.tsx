...
import {searchGroupOptionsProps} from 'types/question';
...
const SearchQuestionGroup: ForwardRefRenderFunction<refProps, props> = (
  props: props,
  ...
  const onFinish = (value: searchGroupOptionsProps) => {
    history.replace({
    ...
    <Drawer
      ...
      <Form layout="vertical" className="h-full relative" onFinish={onFinish}>
        <Row className="d-block h-full overflow-auto px-4 pb-24">
          ...
          <Form.Item name="gap" label={t('gap')} className="mb-1/2 label-p-0" initialValue={queryObject?.gap}>
...
export default forwardRef(SearchQuestionGroup);
