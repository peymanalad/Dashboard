...
const AddOrganizationGlobalUserModal: ForwardRefRenderFunction<refProps, props> = (
  ...
  const sendOrganizationUser = usePost({
    url: 'services/app/OrganizationUsers/CreateGlobal',
    refetchQueries: [['GetGlobalUserLeaves', selectedOrganizationId]],
    form,
    onSuccess: () => {
      form.resetFields();
  ...
  const onFinish = (values: any) => {
    sendOrganizationUser.post({
      ...
      organizationId: selectedOrganizationId ? +selectedOrganizationId : null
    });
  };
...
export default forwardRef(AddOrganizationGlobalUserModal);
