...
const SearchMedicine: ForwardRefRenderFunction<refProps, props> = (
  props: props,
    ...
    <Drawer
      ...
      <Form layout="vertical" className="h-full relative" onFinish={onFinish}>
        <Row className="d-block h-full overflow-auto px-4 pb-24">
          ...
          <Form.Item name="type" label={t('type.title')} className="mb-1/2 label-p-0" initialValue={queryObject?.type}>
            <MultiSelectPaginate
              mode="single"
              url="prescription_types/paginate"
              keyValue="id"
              keyLabel="title"
              urlName="prescriptionTypes"
              placeholder={t('all')}
              isGeneral
              showSearch
            />
...
export default forwardRef(SearchMedicine);
